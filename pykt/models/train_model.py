...
def cal_loss(model, ys, r, rshft, sm, preloss=[], mask_L1=None ):
    ...
    if model_name in ["atdkt", "simplekt", "bakt_time", "sparsekt"]:
        ...
        if model.emb_type.find("predcurc") != -1:
            ...
            loss = loss1
    elif model_name in ["denoisekt"]:
        y = torch.masked_select(ys[0], sm)
        t = torch.masked_select(rshft, sm)
        loss1 = binary_cross_entropy(y.double(), t.double())
        if mask_L1 is not None:
            loss = loss1 + model.lamb * mask_L1
        else:
            loss = loss1
    ...
    elif model_name in ["desaint","desakt"]:
        y = torch.masked_select(ys[0], sm)
        t = torch.masked_select(rshft, sm)
        loss1 = binary_cross_entropy(y.double(), t.double())
        if mask_L1 is not None:
    ...
    elif model_name == "deakt":
        y = torch.masked_select(ys[0], sm)
        t = torch.masked_select(rshft, sm)
        loss1 = binary_cross_entropy(y.double(), t.double()) + preloss[0]
        if mask_L1 is not None:
            loss = loss1 + model.lamb * mask_L1
        else:
            loss = loss1
...
def model_forward(model, data, rel=None):
    ...
    if model_name in ["atdkt"]:
        ...
        y, mask_L1 = model(dcur, train=True)
        ys = [y[:,1:]]
    ...
    if model_name in ["dkt"]:
        ...
        y, mask_L1 = model(c.long(), r.long(), cshft.long())
        ys.append(y)
    elif model_name in ["saint"]:
        y = model(cq.long(), cc.long(), r.long())
        ys.append(y[:, 1:])
    elif model_name in ["desaint"]:
        y, mask_L1 = model(cq.long(), cc.long(), r.long())
        ys.append(y[:, 1:])
    ...
    if model_name in ["desaint","desakt","denoisekt","deakt"]:
        loss = cal_loss(model, ys, r, rshft, sm, preloss, mask_L1)
    elif model_name not in ["atkt", "atktfix"] + que_type_models or model_name in ["lpkt", "rkt"]:
