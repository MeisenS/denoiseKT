...
class TransformerLayer(nn.Module):
    def __init__(self, d_model, d_feature,
        ...
        # Add DAM into Transformer
        self.num_attention_heads = n_heads
        self.init_alphas = nn.Parameter(1e-3 * torch.randn(self.num_attention_heads, 128, 2).to(device))

    def generate_sparse_attention_mask(self, seq_len):
        ...
        mask_tri = torch.zeros(self.num_attention_heads, seq_len, seq_len).to(device)
        mask_tri[:, 0] = mask[:, 0]
        for i in range(1, seq_len):
            mask_tri[:, i, i:] = mask[:, i, :-i]
        masks = mask_tri + torch.transpose(torch.triu(mask_tri, 1), 1, 2)

        return masks.to(dtype=next(self.parameters()).dtype)
    ...
    def forward(self, mask, query, key, values, apply_pos=True):
        ...
        sparse_mask = self.generate_sparse_attention_mask(seqlen)  # 生成稀疏注意力掩码
        nopeek_mask = np.triu(
            np.ones((1, 1, seqlen, seqlen)), k=mask).astype('uint8')
        src_mask = (torch.from_numpy(nopeek_mask) == 0).to(device) & sparse_mask
